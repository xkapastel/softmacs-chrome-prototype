...
class V0 extends lisp.Lisp {
  ...
  init() {
    let scope = new Scope();
    scope.set("pair", this._app2((a, b) => this.pair(a, b)));
    scope.set("fst", this._app1((x) => {
      lisp.guard(isPair, x);
      return x.fst;
    }));
    scope.set("snd", this._app1((x) => {
      lisp.guard(isPair, x);
      return x.snd;
    }));
    ...
    scope.set("wrap", this._app1((x) => this.wrap(x)));
    scope.set("unwrap", this._app1((x) => this.unwrap(x)));
    return scope;
  }
  ...
  _apply(proc, value, scope, rest) {
    ...
    if (isPrimitive(proc)) {
      ...
      let ptree = proc.head;
      if (isSymbol(ptree)) {
        local.set(ptree, value);
      } else {
        while (!isUnit(ptree)) {
          lisp.guard(isPair, ptree);
          lisp.guard(isPair, value);
          lisp.guard(isSymbol, ptree.fst);
          local.set(ptree.fst, value.fst);
          ptree = ptree.snd;
          value = value.snd;
        }
      }
